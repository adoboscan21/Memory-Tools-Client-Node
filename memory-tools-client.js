...
class MemoryToolsClient {
    ...
    this.connectingPromise = new Promise((resolve, reject) => {
      ...
      this.socket = tls.connect(options, () => {
        if (this.socket.authorized || !this.rejectUnauthorized) {
          ...
          reject(
            new Error(
              `DBClient: TLS connection unauthorized: ${this.socket.authorizationError}`
            )
          );
        }
      });
  ...
  async sendCommand(commandType, payloadBuffer) {
  ...
  async collectionCreate(collectionName) {
  ...
  async collectionDelete(collectionName) {
  ...
  async collectionItemSet(collectionName, key, value, ttlSeconds = 0) {
  ...
  async collectionItemGet(collectionName, key) {
  ...
  async collectionItemDelete(collectionName, key) {
  ...
  async collectionItemList(collectionName) {
    ...
    for (const key in rawMap) {
      try {
        ...
        decodedMap[key] = rawMap[key]; // Keep raw Base64 if parsing fails
      }
    }
