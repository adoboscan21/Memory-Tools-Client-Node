...
async function runTests() {
	...
	try {
		...
		const manyItems = [
			{ name: "Node.js Item A", value: 100 },
			{ name: "Node.js Item B", value: 200 },
			{ name: "Node.js Item C", value: 300, _id: "explicit_id" },
		];
		...
		// Verify that all items, old and new, exist
		...
		console.log(`All items in collection after set many:`, allItemsAfterSetMany.items);

		const expectedKeys = [itemKey1, itemKey2, "explicit_id"];
		const foundKeys = Object.keys(allItemsAfterSetMany.items);

		if (foundKeys.length === 5 && foundKeys.includes("explicit_id")) {
			console.log("✔ Success: All expected keys found after set many.");
		} else {
			console.log("✖ Error: Some keys are missing or unexpected keys are present after set many.");
		}
		...
		// Note: The count is now 5 due to the 'set many' test
		if (
			Object.keys(itemsList.items).length === 5 &&
			itemsList.items[itemKey1] &&
			itemsList.items[itemKey2] &&
			itemsList.items["explicit_id"]
		) {
			console.log(
				`✔ Success: All expected items are listed after set many.`
			);
		} else {
			console.log(
		...
		await clientAdmin.collectionItemDelete(testCollectionName, "explicit_id");
		console.timeEnd("CollectionItem_Delete_SetMany");
		console.log(
			`✔ Success: Item 'explicit_id' deleted from '${testCollectionName}'.`
		);
		...
		// The original count was 5 (2 old + 3 new). Now we expect 3 items remaining.
		const remainingKeys = Object.keys(remainingItems.items);
		if (
			remainingKeys.length === 3 &&
			!remainingKeys.includes(itemKey1) &&
			!remainingKeys.includes("explicit_id")
		) {
