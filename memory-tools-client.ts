...
export interface Query {
    ...
    offset?: number;
    count?: boolean;
    aggregations?: { [key: string]: { func: "sum" | "avg" | "min" | "max" | "count"; field: string } };
    groupBy?: string[];
...
export class MemoryToolsClient {
    ...
    public async collectionIndexCreate(collectionName: string, fieldName: string): Promise<string> {
    ...
    public async collectionIndexDelete(collectionName: string, fieldName: string): Promise<string> {
        const payload = Buffer.concat([writeString(collectionName), writeString(fieldName)]);
    ...
    public async collectionItemSet<T = any>(collectionName: string, key: string, value: T, ttlSeconds: number = 0): Promise<string> {
    ...
    public async collectionItemSetMany<T extends { _id?: string }>(collectionName: string, items: T[]): Promise<string> {
    ...
    public async collectionItemUpdate<T = any>(collectionName: string, key: string, patchValue: Partial<T>): Promise<string> {
        const payload = Buffer.concat([
            writeString(collectionName),
            writeString(key),
            writeBytes(Buffer.from(JSON.stringify(patchValue)))
        ]);
        const response = await this.sendCommand(CMD_COLLECTION_ITEM_UPDATE, payload);
        if (response.status !== STATUS_OK) throw new Error(`Item Update failed: ${getStatusString(response.status)}: ${response.message}`);
        return response.message;
    }

    /** Partially updates multiple items in a single batch. `items` must be `[{'_id': 'key1', 'patch': {...}}, ...]`. */
    public async collectionItemUpdateMany<T = any>(collectionName: string, items: { _id: string, patch: Partial<T> }[]): Promise<string> {
        const payload = Buffer.concat([writeString(collectionName), writeBytes(Buffer.from(JSON.stringify(items)))]);
        const response = await this.sendCommand(CMD_COLLECTION_ITEM_UPDATE_MANY, payload);
        if (response.status !== STATUS_OK) throw new Error(`Item Update Many failed: ${getStatusString(response.status)}: ${response.message}`);
        return response.message;
    }

    /** Retrieves a single item from a collection. */
    public async collectionItemGet<T = any>(collectionName: string, key: string): Promise<GetResult<T>> {
    ...
    public async collectionItemDelete(collectionName: string, key: string): Promise<string> {
        const payload = Buffer.concat([writeString(collectionName), writeString(key)]);
    ...
    public async collectionItemDeleteMany(collectionName: string, keys: string[]): Promise<string> {
     ...
     */
    public async collectionItemList<T = any>(collectionName: string): Promise<CollectionItemList<T>> {
        ...
        const decodedMap: CollectionItemList<T> = {};
    ...
    public async collectionQuery<T = any>(collectionName: string, query: Query): Promise<T> {
